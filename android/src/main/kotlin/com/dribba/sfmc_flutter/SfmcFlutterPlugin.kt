...
class SfmcFlutterPlugin : FlutterPlugin, MethodCallHandler {
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        if (call.method == "setContactKey") {
            ...
            val attrName = call.argument<String>("eventName")
            val attrValue = call.argument<Map<String,Any>>("attributes")
            if (attrName == null || attrValue == null) {
                result.error("ARGS_NOT_ALLOWED", "ARGS_NOT_ALLOWED", "ARGS_NOT_ALLOWED");
                return
            }

            result.success(setEventTracking(attrName,attrValue))
        } else {
   ...
   EventTracking
   */
    fun setEventTracking(name: String, attributes : Map<String,Any>): Boolean {
        val event = customEvent(name = name, attributes = attributes)
        SFMCSdk.track(event)
